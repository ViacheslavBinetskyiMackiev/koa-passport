...
function initialize(passport) {
  const middleware = promisify(_initialize(passport, { userProperty: 'passportUser' }))
  return function passportInitialize(ctx, next) {
    // koa <-> connect compatibility:
    const userProperty = passport._userProperty || 'passportUser'
...
function authenticate(passport, name, options, callback) {
  ...
  return function passportAuthenticate(ctx, next) {
    ...
    const p = new Promise((resolve, reject) => {
      // mock the `req` object
      const req = createReqMock(ctx, options.assignProperty || passport._userProperty || 'passportUser')
